...
public class EO3_ValidarExtraccionDeArchivos extends BaseExecution {
	...
	public void test(HashMap<String, String> data) throws Exception {
		...
		/**
		 * ALM
		 *Validar la extraccion de archivos para la plaza 10MEX
		...
		// Paso 6
		String ValidAlmacen = "SELECT ID,ti_id,identificador,fecha_inicial,Fecha_final "
				+ "FROM wmuser.wm_televisa_mul  " + "WHERE ti_id = '%s'";

//		Paso 7
		String ValidMulDetl = "SELECT id,identificador,cr_tienda,nulo,total_venta "
				+ "FROM wmuser.wm_televisa_mul_detl  	" + "WHERE id = '%s'";
...
// *****************************************PASO 1**************************************************************************		
		...
		addStep("Validar que no existan fechas con informaci�n procesada con una antig�edad de 7 d�as.");
		 ...
		 * plaza a procesar. Ruta: /u01/BATCH/EO3/working Host: 10.80.20.11 Port:21
		 * User: wmuser Pass: ?
		 */
		...
		assertFalse(LogRequest, "No se muestra  la informaci�n.");

//		********************************Paso 5 ******************************************************************************

		addStep(" Validar el registro de los archivos descargados en la tabla wm_televisa_inbound_docs y con estatus igual a 'L'.");
		String ID = "";
		String doc_name = "";
		...
		addStep("Validar el almacenamiento del contenido del archivo en la tabla wm_televisa_mul.");
		String ID_Televisa_mul = "";
		System.out.println(GlobalVariables.DB_HOST_Ebs);
		String ValidAlmacenFormat = String.format(ValidAlmacen, ID);
		System.out.println(ValidAlmacenFormat);

		SQLResult ValidAlmacenExec = dbEbs.executeQuery(ValidAlmacenFormat);

		boolean ValidAlmacenbRes = ValidAlmacenExec.isEmpty();

		if (!ValidAlmacenbRes) {

			testCase.addQueryEvidenceCurrentStep(ValidAlmacenExec);
			ID_Televisa_mul = ValidAlmacenExec.getData(0, "ID");
			System.out.println("ID_Televisa_mul: " + ID_Televisa_mul);
		}

		System.out.println(ValidAlmacenbRes);

		assertFalse("No se encontraron registros actualizados", ValidAlmacenbRes);

		// ************************************** Paso 7

		addStep("Validar el almacenamiento del detalle del archivo en la tabla wm_televisa_mul_detl.");

	
		String ValidMulDetlFormat = String.format(ValidMulDetl, ID_Televisa_mul);
		System.out.println(ValidMulDetlFormat);

		SQLResult ValidMulDetlExec = dbEbs.executeQuery(ValidMulDetlFormat);

		boolean ValidMulDetlRes = ValidMulDetlExec.isEmpty();

		if (!ValidMulDetlRes) {

			testCase.addQueryEvidenceCurrentStep(ValidMulDetlExec);
		}

		System.out.println(ValidMulDetlRes);

		assertFalse("No se encontraron registros actualizados", ValidMulDetlRes);
...
//		************************* Paso 9 ***************************************************************
		...
		/**
		 * Validar que el archivo descargado sea respaldado en la carpeta 'backup', el
		 * nombre debe tener concatenada la fecha actual:
		 * [wm_televisa_inbound_docs.doc_name] + 'yyyyMMddhhmmss' Ruta:
		 * /u01/BATCH/EO3/working Host: 10.80.20.11 Port:21 User: wmuser Pass: ?
		 */
//	********************************* Paso 10 **********************************************************
	...
	public String setTestDescription() {
		// TODO Auto-generated method stub
		return "EO3_ValidarExtraccionDeArchivos";
	}
