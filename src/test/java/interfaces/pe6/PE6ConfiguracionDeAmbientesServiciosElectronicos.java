...
public class PE6ConfiguracionDeAmbientesServiciosElectronicos extends BaseExecution {
	...
	public void ATC_FT_004_PE6_Config_Ambiente_Servicios_Electronicos(HashMap<String, String> data) throws Exception {
		...
		// consultas
		String consultaBin = "SELECT DISTINCT PROM_TYPE, BIN,BANK,ACQUIRER,entry_mode, track1, card_type "
				+ "FROM TPEUSER.TDC_TRANSACTION WHERE track1 = '%s'";// FCTDCQA
		String consultaAPPLY_DEP = "SELECT BIN, STATUS, BANK_ID, BANK_DESC, BANK_CODE, APPLY_DEP,CASHBACK "
				+ "from  TPEUSER.TDC_BIN where bin='%s'";// FCTDCQA
		String consultaProc_code = "SELECT * FROM TPEUSER.TDC_ROUTING WHERE PROC_CODE='PAY' and BIN = '%s'";// FCTDCQA
		String tiposDeTransacciones = "select distinct prom_type from TPEUSER.TDC_TRANSACTION "
				+ "where bin='%s' and prom_type is not null and track1 = '%s'";// FCTDCQA
		String consultaTarjetaBloqueada = "select * from TPEUSER.TDC_BLACK_LIST where track1='%s'";// FCTDCQA
		String consultaCuentaBloqueada = "select CREATION_DATE,CARD_NO,BLOQUED_TIME,PLAZA "
				+ "from TPEUSER.TDC_BLOQUED_ACCOUNT where card_no='%s'";// FCTDCQA
		String consultaTiendaBloqueada = "SELECT creation_date,store, card_no, bloqued_time "
				+ "FROM TPEUSER.TDC_BLOQUED_STORE WHERE card_no = '%s'";// FCTDCQA
		...
		testCase.addBoldTextEvidenceCurrentStep("Encontrar los tipos de transacciones que el Bin de la tarjeta= '"
				+ data.get("cardNo") + "'en la tabla TPEUSER.TDC_TRANSACTION.");
		...
		testCase.addBoldTextEvidenceCurrentStep(
				"Validar que la tarjeta puede realizar depositos y retiros, verificando que el campo 'App_dep' y "
				+ "'cashback' esten en Y en la tabla TPEUSER.TDC_BIN.");
		...
		testCase.addBoldTextEvidenceCurrentStep(
				"Valida que la tarjeta pueda realizar pagos, verificando que el campo 'Proc_code' este en PAY "
				        + "y verificar si la tarjeta aplica en todas las plazas y tiendas comprobando "
	    ...
	    // Paso 4 *********************************************************
			...
			testCase.addBoldTextEvidenceCurrentStep(
					"Valida que la tarjeta esta en la lista negra en la tabla TPEUSER.TDC_BLACK_LIST.");
			...
			SQLResult resultadoExecuteConsultaTarjetaBloqueada = dbFCTDCQA
					.executeQuery(executeConsultaTarjetaBloqueada);
			boolean validaTarjeta = resultadoExecuteConsultaTarjetaBloqueada.isEmpty();
			System.out.println(validaTarjeta);

			if (!validaTarjeta) {
				testCase.addTextEvidenceCurrentStep(
						"-La tarjeta esta registrada en la lista negra por lo tanto esta bloqueada.");
				// testCase.addQueryEvidenceCurrentStep(resultadoExecuteConsultaTarjetaBloqueada);

			} else {
				testCase.addTextEvidenceCurrentStep(
						"-La tarjeta no esta registrada en la lista negra por lo tanto no esta bloqueada.");
				// testCase.addQueryEvidenceCurrentStep(resultadoExecuteConsultaTarjetaBloqueada);
			...
			testCase.addBoldTextEvidenceCurrentStep(
					"Valida que la tarjeta ha sido bloqueada en las tiendas en la tabla TPEUSER.TDC_BLOQUED_STORE.");
			...
			testCase.addBoldTextEvidenceCurrentStep(
					"Valida que la tarjeta esta registrada con una cuenta bloqueada en la tabla TPEUSER.TDC_BLOQUED_ACCOUNT.");
		...
		addStep("Encontrar los tipos de transacciones que el Bin de la tarjeta= '" + data.get("cardNo")
				+ "' puede realizar mediante la siguiente consulta en la tabla TPEUSER.TDC_TRANSACTION.");
		...
		addStep("Validar que la tarjeta puede realizar depositos y retiros, verificando que el campo "
				+ "'App_dep' y 'cashback' esten en Y en la tabla TPEUSER.TDC_BIN.");
...
//ssh
		try {
			...
			if (sshConnector.isConected()) {
				...
				if (resumen == true) {
					...
					testCase.addBoldTextEvidenceCurrentStep(
							"Buscar la configuracion del channel del banco '" + banco + "'.");
				...
				if (result == "") {
					...
					testCase.addBoldTextEvidenceCurrentStep(
							"-Nota: el host y el puerto utilizados son los que aparecen en 'value=', sin modificaciones o '_' agregados. ");
	...
	public String setTestFullName() {
		return "ATC_FT_004_PE6_Config_Ambiente_Servicios_Electronicos";
	}
	@Override
