...
public class ATC_FT_016_TPE_FCP_WMx86_TransaccionReposicionSaldazo extends BaseExecution{

	@Test(dataProvider = "data-provider")
	public void ATC_FT_016_TPE_FCP_WMx86_TransaccionReposicionSaldazo_test(HashMap<String, String> data) throws Exception {

		/**
		 * Proyecto: Servicios Electronicos (Regresion Enero 2023)
		 * Caso de prueba: MTC-FT-051 Transacción exitosa de Reposición de tarjeta Saldazo.
		 * @author 
		 * @date 
		 */
		...
		String ValidOperation = "SELECT CARD_CODE,CARD_NUMBER,STATUS,CREATION_DATE,OPERATION,PLAZA,TIENDA "
				+ "FROM TPEUSER.TPE_FCP_CARD  " 
				+ "WHERE PLAZA = '"+data.get("plaza")+"'  "
				+ "AND TIENDA = '"+data.get("tienda")+"' "
				+ "AND CARD_NUMBER ='"+data.get("cardNo")+"' "
				+ "AND STATUS = 'A' "
				+ "ORDER BY CREATION_DATE DESC";
	
		String ValidOperation2 = "SELECT CARD_CODE, STATUS, OPERATION, CREATION_DATE,LOG_ID "
				+ "FROM TPEUSER.TPE_FCP_CARD_LOG  " + "WHERE CREATION_DATE >= trunc(sysdate)  "
				+ "AND OPERATION= 'ASO'  " + "AND card_code='%s' " + "ORDER BY CREATION_DATE DESC";
		...
		// ************************************************************************************************/
		// Paso 1
		addStep("Ejecutar el servicio de solicitud de folio para una transacción exitosa de reposición de tarjeta Saldazo");
		...
		addStep("Ejecutar en el navegador el servicio para realizar la solicitud de autorización de una reposición de tarjeta saldazo");
		System.out.println("Paso 2: ");
		
		String respuestaAut = FCPUtil.generarAutorizacionREP(folio,CreationDate);
		 ...
		 * Paso 5
		 *****************************************************************************************/
		addStep("Ejecutar consulta para validar que se registró correctamente la transacción de reposición de tarjeta Saldazo en site 1 MTY: ");
		
		// Se forma el query a utilizar
		System.out.println("Paso 5: "+GlobalVariables.DB_HOST_FCTPE);
...
//	***************************Consulta en SIte 2 QRO*************************************************	
		testCase.addBoldTextEvidenceCurrentStep("Ejecutar consulta para validar que se registró correctamente "
				+ "transacción de reposición de tarjeta Saldazo en FCP en site 2 QRO: " + GlobalVariables.DB_HOST_TPE_LOT);
		 ...
		 * Paso 6
		 *****************************************************************************************/
		addStep(" Ejecutar la siguiente consulta para validar que el registro de la tarjeta en la tabla TPEUSER.TPE_FCP_CARD con STATUS = 'A'. en site 1 MTY.  ");

		System.out.println("Paso 6: "+GlobalVariables.DB_HOST_FCTPE);
		String ValidFormat = String.format(ValidOperation);
		...
		String CARD_CODE="";
		if (!ValidTAEVal) {
			CARD_CODE=ValidExec.getData(0, "CARD_CODE");
			testCase.addTextEvidenceCurrentStep("Se encontraron resultados recientes con STATUS = 'A'");
		}
...
//		***************************COnsulta site 2 FCTPE_QRO******************************************************
		...
		assertFalse(ValidTAEVal, "No se encontraron resultados recientes con STATUS = 'A'");
...
//		***************************************************
		...
		addStep(" Ejecutar la siguiente consulta, para validar que el registro de la tarjeta activa haya cambiado a OPERATION= 'ASO' en site 1 MTY.  ");

		System.out.println(GlobalVariables.DB_HOST_FCTPE);
		String ValidFormat2 = String.format(ValidOperation2, CARD_CODE);
		System.out.println(ValidFormat2);

		SQLResult ValidExec3 = executeQuery(dbTPEMTY, ValidFormat2);

		boolean ValidTAEVal2 = ValidExec3.isEmpty();

		if (!ValidTAEVal2) {
			testCase.addTextEvidenceCurrentStep("Se encontraron resultados recientes con operacion ASO");
		}
		testCase.addQueryEvidenceCurrentStep(ValidExec3);

//		***************************COnsulta site 2 FCTPE_QRO******************************************************
		System.out.println(GlobalVariables.DB_HOST_TPE_LOT);
		SQLResult ValidExec4 = executeQuery(dbTPEQRO, ValidFormat2);
		testCase.addQueryEvidenceCurrentStep(ValidExec4);

		assertFalse(ValidTAEVal2, "No se encontraron resultados recientes con operacion ASO");
	...
	public String setTestFullName() {
		return "ATC_FT_016_TPE_FCP_WMx86_TransaccionReposicionSaldazo_test";
	}

	@Override
	public String setTestDescription() {
		// TODO Auto-generated method stub
		return "Construido. MTC-FT-051 Transaccion exitosa de Reposicion de tarjeta Saldazo";
	}
