...
public class SB_WO1_VerificarProcesoSincronizarConveniosCancelacion extends BaseExecution{

	/**
	 * Desc:Verificar proceso de la interfaz modulo - Sincronizar Convenios Cancelaciones Web Con Oracle
	 * @author Ultima modificacion Mariana Vives
	 * @date 27/02/2023
	 */
	
	@Test(dataProvider = "data-provider")
	public void SB_WO1_VerificarProcesoSincronizarConveniosCancelacion_test(HashMap<String, String> data) throws Exception {
		...
		// Paso 1
		String VerifDatos="SELECT numero_convenio, TO_CHAR(fecha_cancelacion,'RRRRMMDD'), wm_status "
...
//	Paso 4
		String VerifUpdt="SELECT NUMERO_CONVENIO,TIPO_CONVENIO_LOOKUP,WM_STATUS,TO_CHAR(fecha_cancelacion,'RRRR-MM-DD') as FECHA_CANCELACION "
				+ " FROM XXBN.xxbn_convenios "
				+ " WHERE wm_status = 'C' "
				+ " and numero_convenio='%s' ";
//		PASO 5
		String validFech="SELECT NUMERO_CONVENIO,TIPO_CONVENIO,fecha_cancelacion  "
				+ "FROM XXFC.xxfc_convenios "
				+ "WHERE numero_convenio = '%s' "
				+ "AND fecha_cancelacion >= TO_DATE( '%s', 'RRRR-MM-DD')";
...
//****	
		...
		String UpdStatusConvenios= "update XXBN.xxbn_convenios "
				+ "set wm_status = 'E' "
				+ "where NUMERO_CONVENIO in (2,9)";
...
//		**********************************Update generacion de insumo*****************************************************
		
		System.out.println(GlobalVariables.DB_HOST_FCIASQA);
		System.out.println(UpdStatusConvenios);
		dbFciasqa.executeUpdate(UpdStatusConvenios);
...
////		********************Paso 4***************************************************************
		addStep("Verificar la actualización del status a C de los convenios  en la tabla XXBN_CONVENIOS");
		...
		if (!VerifUpdtFormatRes) {
			FECHA_CANCELACION = ExecVerifUpdtFormat.getData(0, "FECHA_CANCELACION");
	testCase.addQueryEvidenceCurrentStep(ExecVerifUpdtFormat);
		}

		System.out.println(VerifUpdtFormatRes);
		assertFalse(VerifUpdtFormatRes, "No se actualizaron los datos" );

////	********************Paso 5***************************************************************
		

	addStep("Verificar la actualización de la fecha de cancelación de los convenios en la tabla XXFC_CONVENIOS.");

	System.out.println(GlobalVariables.DB_HOST_AVEBQA);
	String validFechFormat = String.format(validFech, NUMERO_CONVENIO, FECHA_CANCELACION);
	System.out.println(validFechFormat);	
	SQLResult ExecvalidFechFormat = dbAvebqa.executeQuery(validFechFormat);
	boolean ExecvalidFechFormatRes = ExecvalidFechFormat.isEmpty();

	if (!ExecvalidFechFormatRes) {

testCase.addQueryEvidenceCurrentStep(ExecvalidFechFormat);
	}

	System.out.println(ExecvalidFechFormatRes);
	assertFalse(ExecvalidFechFormatRes, "No se actualizo la fecha" );
	...
	public String setTestFullName() {
		...
		return "SB_WO1_VerificarProcesoSincronizarConveniosCancelacion_test";
	}

	@Override
	public String setTestDescription() {
		// TODO Auto-generated method stub
		return "SB_WO1_VerificarProcesoSincronizarConveniosCancelacion";
	}
