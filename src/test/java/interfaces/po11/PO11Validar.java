...
public class PO11Validar extends BaseExecution {
	@Test(dataProvider = "data-provider")
	public void ATC_FT_009_PO11_Validar_Archivos(HashMap<String, String> data) throws Exception {
		/*
		 * Utilerías
		 *********************************************************************/
		...
		String tdcQueryValidar = "SELECT substr(pv_doc_name,4,5) AS plaza, COUNT(*) AS RECORDS"
				+ " FROM posuser.pos_inbound_docs A, posuser.plazas b" + " WHERE A.doc_type='DCI'"
				+ " AND A.status = 'I'" + " AND substr(A.pv_doc_name,4,5) = b.cr_plaza" + " AND b.pais= 'MEX'"
				+ " AND TRUNC(A.partition_date) >= TRUNC(sysdate-7)" + " GROUP BY substr(A.pv_doc_name,4,5)"
				+ " ORDER BY RECORDS DESC";
		String tdcQueryProfundidad = "SELECT INTERFASE, ENTIDAD, OPERACION, CATEGORIA, DESCRIPCION"
				+ " FROM WMUSER.wm_interfase_config" + " WHERE interfase = 'PO11'" + " AND operacion = 'DEPTH_DAYS'";
		String tdcQueryIntegrationServer = "select * from ( SELECT run_id,start_dt,status" + " FROM WMLOG.wm_log_run"
				+ " WHERE interface = 'PO11'" + " and  start_dt >= TRUNC(SYSDATE)" + " order by start_dt desc)"
				+ " where rownum = 1";
		String tdcQueryErrorId = " SELECT ERROR_ID,RUN_ID,ERROR_DATE,DESCRIPTION " + " FROM WMLOG.WM_LOG_ERROR "
				+ " where RUN_ID=%s" + " and rownum = 1";
		String tdcQueryPoliza = "SELECT A.ID, A.PE_ID, A.PV_DOC_ID, A.STATUS, A.DOC_TYPE, B.CR_PLAZA, B.PAIS, A.TARGET_ID"
				+ " FROM POSUSER.POS_INBOUND_DOCS A, POSUSER.PLAZAS B" + " WHERE A.DOC_TYPE='DCI'"
				+ " AND A.STATUS = 'E'" + " AND SUBSTR(A.PV_DOC_NAME,4,5) = B.CR_PLAZA" + " AND B.PAIS= 'MEX'"
				+ " AND TRUNC(A.PARTITION_DATE) >= TRUNC(SYSDATE-7)" + " ORDER BY PV_DOC_NAME";
		String tdcQueryFinanzas = "SELECT REFERENCE6 POLIZA,STATUS,USER_JE_CATEGORY_NAME,GROUP_ID, SEGMENT4 NUM_CTA_GRAL,"
				...
				+ " FROM GL.GL_INTERFACE" + " WHERE REFERENCE6 = '%s'" + " ORDER BY DATE_CREATED DESC";
...
// testCase.setPrerequisites("*Establecer la conexión de la BD FCWM6QA"
// + "*Establecer la conexión de la BD WMLOG_FCWMLQA-NEW"
// + "*Establecer la conexión de la BD "+data.get("preRequisitos")+""
// + "*Contar con acceso al servidor desde el Designer"
// + "*Contar con acceso al servidor QA1 ");
		/**
...
// testCase.addBoldTextEvidenceCurrentStep("Se muestra el STATUS = E  de los archivos procesados y el campo TARGET_ID  se muestra el numero de la poliza");
		}
...
// testCase.addBoldTextEvidenceCurrentStep("Resultado Esperado:");
// testCase.addBoldTextEvidenceCurrentStep("Se mostraran los registros insertados en finanzas, se valida que genere una línea contable por cada movimiento a la cuenta 11CIXXXX y realice una línea de cargo a la cuenta 11BAXXXX (de acuerdo al tipo efectivo), se hará un cargo por cada movimiento DEP encontrado.\r\n" +
// "\r\n" +
// "Validar número de cuenta en el campo SEGMENT4(NUM_CTA_GRAL)  y número de póliza en REFERENCE6");
		}
...
// TODO Auto-generated method stub
		return "Terminado. Valida que se inserten datos en finanzas.";
	}
...
// TODO Auto-generated method stub
		return "ATC_FT_009_PO11_Validar_Archivos";
	}
