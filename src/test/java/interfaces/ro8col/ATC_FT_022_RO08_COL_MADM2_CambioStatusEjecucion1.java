...
public class ATC_FT_022_RO08_COL_MADM2_CambioStatusEjecucion1 extends BaseExecution {

	@Test(dataProvider = "data-provider")
	public void ATC_FT_022_RO08_COL_MADM2_CambioStatusEjecucion1_test(HashMap<String, String> data) throws Exception {
		
		/**
		 * Proyecto: Mejoras Administrativo CC3
		   Caso de prueba: ATC-FT-018-Cambio de status para ejecucion de PR24
		   ATC-FT-019-Cambio de status para ejecucion de PR1
		   ATC-FT-020-Cambio de status para ejecucion de PR4
		   ATC-FT-021-Cambio de status para ejecucion de PR23
		   ATC-FT-022-Cambio de status para ejecucion de PR28
		   ATC-FT-024-Cambio de status para ejecucion de PR11
		   ATC-FT-025-Cambio de status para ejecucion de PR37
		 * Desc:
	...
	//Segundo paso    
	 ...
	 //Tercer paso
	    ...
	    String validacionRMS = "select DOC_TYPE, DOC_ID, RUN_ID, STATUS, CR_PLAZA, CR_TIENDA \r\n"
...
//Paso 4 ************************************************************************************************************		
		addStep("Conectarse a la base de datos de RMS Colombia");
		 ...
		 testCase.addBoldTextEvidenceCurrentStep("La conexion fue exitosa");
		 testCase.addTextEvidenceCurrentStep(GlobalVariables.DB_HOST_RMS_COL_QAVIEW);
		 testCase.addTextEvidenceCurrentStep(GlobalVariables.DB_USER_RMS_COL_QAVIEW);
	 
//Paso 5 ************************************************************************************************************	
		 
		 addStep("Validar que la información llegó a RMS por medio de la sigueinte consulta en FCRMSCQA:");	
		 
		 String validacionRMSFormat = String.format(validacionRMS ,run_id);

			System.out.println(validacionRMSFormat);
			
			SQLResult validacionRMS_result = executeQuery( dbRms,validacionRMSFormat);
			
			
			boolean validacionRMSBoolean = validacionRMS_result.isEmpty();
			
			System.out.println("Valida en RMS "+ validacionRMSBoolean);

				if (!validacionRMSBoolean) {
			
					testCase.addQueryEvidenceCurrentStep(validacionRMS_result);
			
				}


		assertFalse(validacionRMSBoolean, "No se otubtuvieron registros en RMS");	
