...
public class PR36ProcesamientoLocationProveedorCol extends BaseExecution {
	@Test(dataProvider = "data-provider")
	public void ATC_FT_005_PR36_PR36ProcesamientoLocationProveedorCol(HashMap<String, String> data) throws Exception {
	
/* Utilerías *********************************************************************/		
			
	//	SqlUtil dbPosUse = new SqlUtil(GlobalVariables.DB_USER_FCWMQA_NUEVA, GlobalVariables.DB_PASSWORD_FCWMQA_NUEVA, GlobalVariables.DB_HOST_FCWMQA_NUEVA);
	//	SqlUtil dbRmsCol = new SqlUtil(GlobalVariables.DB_USER_RMS_COL, GlobalVariables.DB_PASSWORD_RMS_COL, GlobalVariables.DB_HOST_RMS_COL);		
	//	SqlUtil dbLog = new SqlUtil(GlobalVariables.DB_USER_FCWMLQA, GlobalVariables.DB_PASSWORD_FCWMLQA, GlobalVariables.DB_HOST_FCWMLQA);
		utils.sql.SQLUtil dbPosUse = new utils.sql.SQLUtil(GlobalVariables.DB_HOST_FCWMQA_NUEVA,GlobalVariables.DB_USER_FCWMQA_NUEVA, GlobalVariables.DB_PASSWORD_FCWMQA_NUEVA);
		utils.sql.SQLUtil dbRmsCol = new utils.sql.SQLUtil(GlobalVariables.DB_HOST_RMS_COL_QAVIEW,GlobalVariables.DB_USER_RMS_COL_QAVIEW, GlobalVariables.DB_PASSWORD_RMS_COL_QAVIEW);
		...
		String tdcAvailablePlaza = "select LOCATION, SUBSTR(store_name10,1,5) PLAZA from"
			...
			String lnPendientes = "SELECT LPAD (item, 16, ' ') item, primary_repl_supplier,"
				...
				+" FROM (SELECT DISTINCT ril.item, primary_repl_supplier, pct_tolerance,"
				               ...
				               +" ( SELECT VPN FROM ITEM_SUPPLIER itms  WHERE itms.item = ril.item AND itms.supplier = primary_repl_supplier) as "
				               + " VPN"                
//				                +" ,s.fem_rfc_code,"
//				                +" ril.dispatch_unit,"
//				                +" ril.unit_cost,"
...
addStep("Obtener las plazas que serán procesadas (adpGetAvailableStores) con la siguiente consulta:");	
        ...
        SQLResult AvaiblePlaza = executeQuery(dbRmsCol, tdcAvailablePlaza);

//		SQLUtil.executeQuery(testCase, dbRmsCol, tdcAvailablePlaza);
...
addStep("Obtener las lineas pendientes a procesar según la plaza consultada con la siguiente consulta:");
    ...
    SQLResult ExeInpendientes = executeQuery(dbRmsCol, lnPendientes);

//	SQLUtil.executeQuery(testCase, dbRmsCol, lnPendientes);
...
//		String regPlazaTienda = SQLUtil.getColumn(testCase, dbLog, queryStatusThread, "STATUS");
		String  regPlazaTienda = StatusThread.getData(0, "STATUS");
 ...
 */
	...
	SQLResult PosPutBound = executeQuery(dbPosUse, tdcOutbound);

//	SQLUtil.executeQuery(testCase, dbPosUse, tdcOutbound);

	boolean ou = PosPutBound.isEmpty();
	...
	public String setTestFullName() {
		...
		return "ATC_FT_005_PR36_PR36ProcesamientoLocationProveedorCol";
	}
