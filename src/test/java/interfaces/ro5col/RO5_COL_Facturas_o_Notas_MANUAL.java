...
public class RO5_COL_Facturas_o_Notas_MANUAL extends BaseExecution {
	
	@Test(dataProvider = "data-provider")
	public void ATC_FT_002_RO5_COL_Ejecucion_Interfaz_Procesamiento_Modo_MANUAL(HashMap<String, String> data) throws Exception {
...
*/
		...
		String procesoRtvhead ="SELECT RTV_ORDER_NO, SUPPLIER_ID, STORE, COMPLETED_DATE, REFERENCE_3, REFERENCE_8"
				+ "FROM fem_rechead_temp " + 
				" WHERE TRUNC(receive_date) BETWEEN "
				+ " TO_DATE('%s','DD/MM/YYYY')" + 
				" AND TO_DATE('%s','DD/MM/YYYY')" + 
		...
		String wm_cfg_launcher ="SELECT * FROM wmuser.wm_cfg_launcher " + 
				...
				" AND manual_status = 'I' AND attribute1 = "+ "'"+ data.get("plaza") + "'" ;
...
//				Paso 3	************************	
		addStep("Verificar que la plaza " + data.get("plaza")+ " está configurada para su procesamiento en la tabla wm_cfg_launcher de WMINT en modo MANUAL.");
...
//		Paso 4	************************	
		...
		addStep("Ejecutar el servicio RO5_COL.Pub:run asignando al parámetro type el valor MANUAL.");
...
//		Paso 7  *************************************************
		...
		String procesoRechead2 = String.format(procesoRechead, fechaMin,fechaMax);
		System.out.println(procesoRechead2);
		
		 SQLResult RHD = dbRMSCOL.executeQuery(procesoRechead);
		 ...
		 assertFalse(existeRegistro, "No existen datos a procesar en fem_rechead_temp");
	...
	//****************************************************************		
		...
		String procesoRtvhead2 = String.format(procesoRtvhead, fechaMin,fechaMax);
		System.out.println(procesoRtvhead2);
		
		 SQLResult RTV = dbRMSCOL.executeQuery(procesoRtvhead);

		 String ref3_RTV = RTV.getData(0, "reference_3");

		
		 boolean existeRegistro2 = RTV.isEmpty();
		
		if (!existeRegistro2 ) {
			
			testCase.addQueryEvidenceCurrentStep(RTV);

		}
		
		assertFalse(existeRegistro2, "No existen datos a procesar en fem_rtvhead_temp");
		 System.out.println(existeRegistro2);
	...
	public String setTestFullName() {
		...
		return "ATC_FT_002_RO5_COL_Ejecucion_Interfaz_Procesamiento_Modo_MANUAL";
	}

	@Override
	public String setTestDescription() {
		// TODO Auto-generated method stub
		return "Construido. Ejecución de la interfaz para el procesamiento de las Facturas o Notas en modo MANUAL.";
	}
