...
public class ATC_FT_EO08_005_ProcesamientoInterfazReProcesoPlaza extends BaseExecution {

	@Test(dataProvider = "data-provider")
	public void ATC_FT_EO08_005_ProcesamientoInterfazReProcesoPlaza_test(HashMap<String, String> data) throws Exception {
		...
		/**
		 * ALM
		 * Verificar procesamiento de la interfaz de REPROCESO para la plaza 10CUE.
		 * NOTA: Falta ultimo paso
		 */
		...
		String tdcQueryEpaHeader = "SELECT GL_JOURNAL_ID FROM epa_header"
				+ " WHERE to_char(wm_receive_date,'dd/mm/yyyy') = to_char(sysdate,'dd/mm/yyyy')"
				+ " AND wm_status_inserted = 'L' " 
				+ " AND wm_status_sent = 'L'" 
		...
		String tdcQueryxxfcglinterface = "SELECT * FROM xxfc_gl_interface WHERE xxfc_estatus = 'S'" 
		        + "AND segment3 = '"+ data.get("plaza") + "'" 
				+ "AND TRUNC(date_created) >= TRUNC(SYSDATE)" 
...
//	 	Paso 1	************************
		...
		if (!config) {
			...
			System.out.println(tdcQueryEpaHeader);

			SQLResult EpaHeader = executeQuery(dbEbs, tdcQueryEpaHeader);

			boolean EpaHeader1 = EpaHeader.isEmpty();
			System.out.println(EpaHeader1);

			if (!EpaHeader1) {
				testCase.addQueryEvidenceCurrentStep(EpaHeader);
				gl_journal_id = EpaHeader.getData(0, "GL_JOURNAL_ID");
			}
			assertFalse(EpaHeader1, "No se obtiene informacion de la consulta");
		}
...
//	 	Paso 2	************************
		...
		addStep("Ejecutar el servicio EO08.Pub:runReproc desde el Job EO08");
		...
		boolean queryIntegracion = query.isEmpty();
		...
		assertFalse(queryIntegracion, "No se obtiene informacion de la consulta");
...
//	 	Paso 4	************************
		...
		addStep("Validar que se inserte el detalle de la ejecuci�n de los Threads lanzados por la ejecucion de la interface en la tabla WM_LOG_THREAD de WMLOG.");
...
//	 	Paso 5	************************
		...
		addStep("Verificar la actualizacion de los estatus en la tabla EPA_HEADER, wm_status_inserted=P y wm_status_sent=P.");
...
//	 	Paso 6	************************
		...
		addStep("Verificar la insercion de polizas contables en la tabla GL_INTERFACE");
...
//	 	Paso 7	************************	
		...
		addStep("Verificar la actualizacion de los datos en la tabla XXFC_GL_INTERFACE, xxfc_estatus = S");
...
//      Paso 8 **************************
	...
	public String setTestDescription() {
		// TODO Auto-generated method stub
		return "Verificar procesamiento de la interfaz de REPROCESO para la plaza 10Mon";
	}
	...
	public String setTestFullName() {
		// TODO Auto-generated method stub
		return "ATC_FT_EO08_005_ProcesamientoInterfazReProcesoPlaza_test";
	}
