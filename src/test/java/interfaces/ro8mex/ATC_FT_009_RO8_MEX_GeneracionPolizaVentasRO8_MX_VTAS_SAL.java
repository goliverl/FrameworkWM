...
public class ATC_FT_009_RO8_MEX_GeneracionPolizaVentasRO8_MX_VTAS_SAL extends BaseExecution {
	
	@Test(dataProvider = "data-provider")
	public void ATC_FT_009_RO8_MEX_GeneracionPolizaVentasRO8_MX_VTAS_SAL_test(HashMap<String, String> data) throws Exception {
		/**
		 * Proyecto: BackOffice Oracle
		 * Caso de prueba: MTS-FT-009-C1 RO8_MX Generacion de poliza de ventas por medio 
		 * de la interface RO8_MX_VTAS SAL(1 y 4).
...
//Paso 2
		String consultaPlazas = "SELECT INTERFACE_NAME,AUTO_STATUS,MANUAL_STATUS,ATTRIBUTE1,ATTRIBUTE2 "
				...
				+ "AND ATTRIBUTE2 IN (1,4) AND ATTRIBUTE1='10AGC'";
//Paso 4
		String consultaRegistros = "SELECT * FROM("
				+ "SELECT ITEM, ITEM_DESC,DIVISION,GROUPS,DEPT,CLASS,TRAN_DATE,ID FROM FEM_FIF_STG "
				+ "WHERE REFERENCE_3 IS NULL AND REFERENCE_9 IS NULL AND CR_PLAZA='10AGC' AND TRAN_CODE IN ('1','4') "
...
//Paso 7 
		String registrosItem = "SELECT * FROM("
				...
				+ "FROM FEM_FIF_STG WHERE CR_PLAZA='10AGC' AND TRAN_CODE IN ('1','4') AND ID = '%s'"
				+ "ORDER BY TRAN_DATE DESC)"
				+ "WHERE ROWNUM <= 10";
//Paso 9
		String validarRegistros = "SELECT * FROM GL.GL_INTERFACE\r\n"
				+ "WHERE DATE_CREATED >= <DD/MM/AAAA>'\r\n"
				+ "AND GROUP_ID IN (<GROUP_ID>)\r\n"
				+ "ORDER BY DATE_CREATED DESC"; //LA MATRIZ NO EXPLICA DE DONDE SALE EL GROUP ID
//Paso 11
