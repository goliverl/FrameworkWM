package interfaces.ro4;
	...
	public class RO4_MEX_NOTAS_CREDITO_AUTO extends BaseExecution {
		
		@Test(dataProvider = "data-provider")
		public void ATC_FT_RO4_003_MEX_NOTAS_CREDITO_AUTO(HashMap<String, String> data) throws Exception {
	...
	//***********************************************************************************************************************************************************************		
			...
			String datosPtesProcesar1= "SELECT* FROM (SELECT completed_date, ret_auth_num, handling_pct, handling_cost, comment_desc FROM RMS100.FEM_RTVHEAD_TEMP WHERE trunc(completed_date) "
			...
			String datosPtesProcesar2 = "SELECT* FROM (SELECT reference_1, reference_2, reference_3, reference_4, reference_5, reference_6 FROM RMS100.FEM_RTVHEAD_TEMP WHERE trunc(completed_date) "
					+ "BETWEEN to_date ('%s','dd,mm,yyyy') AND to_date('%s','dd,mm,yyyy') AND reference_3 IS NULL AND reference_8 IS NULL AND "
					+ "store IN (SELECT store FROM store WHERE substr(store_name10, 1,5) = '"+data.get("plaza")+"'))WHERE rownum <= 1 ";
			
			
			String datosPtesProcesar3 = "SELECT* FROM (SELECT reference_7, reference_8, reference_9, reference_10, merch_type, fem_remision FROM RMS100.FEM_RTVHEAD_TEMP WHERE trunc(completed_date) "
					+ "BETWEEN to_date ('%s','dd,mm,yyyy') AND to_date('%s','dd,mm,yyyy') AND reference_3 IS NULL AND reference_8 IS NULL AND "
					+ "store IN (SELECT store FROM store WHERE substr(store_name10, 1,5) = '"+data.get("plaza")+"'))WHERE rownum <= 1 ";
	...
	//********************************************************************************************************************************************************************************		
			...
			String verificaConfig= "SELECT * FROM WMUSER.wm_cfg_launcher WHERE interface_name = 'RO4' AND auto_status = 'A' AND attribute1 = '"+ data.get("plaza")+"'";
	...
	//*************************************************************************************************************************************************************************
			...
			String tdcIntegrationServerFormat = "	select * from (SELECT Tbl.run_id,interface, start_dt, end_dt, status, server, (END_DT - START_DT)*24*60 "
					+ "FROM WMLOG.WM_LOG_RUN Tbl " + "WHERE INTERFACE LIKE '%RO4%' "
	...
	//*******************************************************************************************************************************************************************
			...
			String datos_invoice_interface = "SELECT invoice_id, invoice_line_id, line_number, line_type_lookup_code, line_group_number, amount FROM ap.ap_invoice_lines_interface WHERE invoice_id = '%s' " ;
			
			String datos_invoice_line_interface = "SELECT invoice_id, invoice_line_id, line_number, line_type_lookup_code, line_group_number, amount FROM ap.ap_invoice_lines_interface WHERE invoice_id = '%s' ";
	...
	//********************************************************************************************************************************************************************************
	        ...
	        addStep("Obtener el rango de fechas a procesar por la interfaz RO4_MEX:"
	        		...
	        		+ "FROM wm_cat_ro4_weeks WHERE max_date < SYSDATE AND week_nb IN "
	        		+ "nvl((SELECT DISTINCT MAX(decode(status,'E',week_nb,'S',week_nb+1,1)) "
	        		+ "FROM wm_log_ro4 WHERE cr_plaza = '"+ data.get("plaza")+"' AND week_nb IN (SELECT DISTINCT MAX(week_nb) "
	        		+ "FROM wm_log_ro4 WHERE cr_plaza = '10MEX') ),1);");        
	       ...
	       assertFalse("No se muestran registros que cumplan el rango de ejecucuion.", checarFechas ); //Si esta vacio, imprime mensaje
	...
	//*****************************************************************************************************************************************************************************
	      ...
	      addStep("Verificar que existen datos pendientes por procesar en la tabla fem_rtvhead_temp de RETEK para la plaza"+ data.get("plaza")+" con el rango de fechas obtenido: "
	      		+ "SELECT * FROM fem_rtvhead_temp WHERE trunc(completed_date) BETWEEN [min_date] AND [max_date] "
	      		+ "AND reference_3 IS NULL AND reference_8 IS NULL AND store IN (SELECT store FROM store "
	      		+ "WHERE substr(store_name10, 1,5) = '"+ data.get("plaza")+"');");        
	        ...
	        SQLResult consultaDatos1= dbRms.executeQuery(consulta1); //recibe la consulta con las fechas maxima y minima.
	       ...
	       //Parte 2
	        ...
	        SQLResult consultaDatos2 = dbRms.executeQuery(consulta2);
            ...
            SQLResult consultaDatos3 = dbRms.executeQuery(consulta3);
            ...
            SQLResult consultaDatos4 = dbRms.executeQuery(consulta4);
	       ...
	       assertFalse("No se muestran registros que cumplan con el rango de fechas en fem_rechead_temp", validaConsulta4);
	 ...
	 addStep("Verificar que la plaza "+ data.get("plaza")+" está configurada para su procesamiento en la tabla wm_cfg_launcher de WMINT en modo AUTO: "
	 		+ "SELECT * FROM wm_cfg_launcher WHERE interface_name = 'RO4' AND auto_status = 'A' AND attribute1 = '"+ data.get("plaza")+"';" );        

	       
	       System.out.println(GlobalVariables.DB_HOST_FCWMQA_NUEVA); //posuser
	       
	      //Parte 1
	       System.out.println(verificaConfig);
	       
	       SQLResult verificaConfigDatos = dbPos.executeQuery(verificaConfig);
	      ...
	      assertFalse("No se muestran registros que cumplan con el rango de fechas en la tabla FEM_RTVHEAD_TEM", consultaConfig);
	 ...
	 //************************************************************************************************************************************************************************************  
	     ...
	     // Utileria
	      ...
	      String dateExecution = pok.runIntefaceWmWithInput(data.get("interfase"), data.get("servicio"),data.get("TYPE"),"TYPE");
	...
	//************************************************************************************************************************************************************************************
		...
		//Paso 6
		        ...
		        addStep("Verificar que los datos de la tabla fem_rtvhead_temp de RETEK fueron procesados correctamente: "
		        		+ "SELECT * FROM fem_rtvhead_temp WHERE RTV_ORDER_NO= [rtv_order_no] AND reference_3 IS NOT NULL "
		        		+ "AND reference_8 IS NOT NULL AND store IN (SELECT store FROM store WHERE substr(store_name10, 1,5) = '"+ data.get("plaza")+"');");
		        ...
		        SQLResult consultaRTV = dbRms.executeQuery(consulta_rtvorder);
		        ...
		        SQLResult consultaRTV2 =dbRms.executeQuery(consulta_rtvorder2);
		        ...
		        SQLResult consultaRTV3= dbRms.executeQuery(consulta_rtvorder3);
	...
	//*****************************************************************************************************************************************************************************	              
                ...
                SQLResult invoice_interface= dbEbs.executeQuery(consulta_ap_invoices_interface);
				...
				assertFalse(valida_Invoice_Interface, "No se encontraron facturas y notas en la tabla ap_invoices_interface");
				...
				addStep("Verificar que las Facturas fueron insertadas en la tabla ap_invoice_lines_interface de ORAFIN: "
		        		+ "SELECT * FROM ap.ap_invoice_lines_interface WHERE invoice_id = [fem_rtvhead_temp.reference_3]; ");
				...
				System.out.println(consulta_invoices_line_interface);
				 ...
				 SQLResult invoice_interface_lines= dbEbs.executeQuery(consulta_invoices_line_interface);
				...
				assertFalse(valida_Invoice_Interface_lines, "No se encontraron facturas y notas en la tabla ap_invoice_lines_interface");
		...
		public String setTestFullName() {
			...
			return "ATC_FT_RO4_003_MEX_NOTAS_CREDITO_AUTO";
		}

		@Override
		public String setTestDescription() {
			// TODO Auto-generated method stub
			return "Ejecución de la interfaz para el procesamiento de las Notas de crédito en modo AUTO";
		}
