...
public class ATC_FT_002_TPE_LOY_TransacAcumulExitosa extends BaseExecution {

	@Test(dataProvider = "data-provider")
	public void ATC_FT_002_TPE_LOY_TransacAcumulExitosa_test(HashMap<String, String> data) throws Exception {
		
		/**
		 * TPE_LOY: MTC-FT-002 TPE_LOY Transacción de acumulacion exitosa
		 * Desc: Prueba de regresion para comprobar la no afectación en la funcionalidad 
		 * de acumulación exitosa de la interface FEMSA_TPE_LOY al ser migrada de webmethods 
		...
		addStep("Establecer conexion con la BD 'OXTPEQA' ");
		...
		addStep("Validar el registro de la transaccion de acumulacion de puntos");
		...
		if(!validaConsultaAcumulacionPts) {
			...
			wm_code = consultaRefAcumulacionPts_r.getData(0, "WM_CODE");
		...
		assertEquals(expect_wm_code,wm_code);
		
		
		// Paso 6 ****************************************************
		/*
		addStep("Establecer conexion con la BD 'OXWMLOGQA' ");
		...
		addStep("Validar que se registró la ejecución hecha por "
				+ "el canal no seguro");
	...
	public String setTestFullName() {
		return "ATC_FT_002_TPE_LOY_TransacAcumulExitosa_test";
	}
