...
public class OR1_OG_RegistrarInfoDeAjustesInventario extends BaseExecution{
	
	@Test(dataProvider = "data-provider")
	public void ATC_FT_002_OR1_OG_Registrar_Info_Ajustes_Inventarios(HashMap<String, String> data) throws Exception {
...
* 
*/		
		String tdcQueryInventarios = "SELECT  ASD_ITEM, ASD_ITEM_DESC, ASD_TRAN_DATE, ASD_CR_PLAZA, WM_STATUS, WM_RUN_ID "
				+ " FROM XXFC_AJUSTE_SALDOS_AUTOMATICOS"
				+ " WHERE WM_STATUS = '"+ data.get("status") +"' "
...
//Paso 1 *************************		
		addStep("Validar que existan información de ajustes de inventarios para la plaza "+ data.get("plaza") +" pendiente por procesar en la tabla XXFC_AJUSTE_SALDOS_AUTOMATICOS de ORACLE AR.");
		...
		assertFalse(inventarios, "No se obtiene información de la consulta");

		
//Paso 2 *************************		
		addStep("Ejecutar la interface OR1_OG.Pub:run para enviar la información de los ajustes de inventarios de ORACLE AR a RETEK para la plaza "+ data.get("plaza") +".");
...
//		pok.runIntefaceWmWithInput10(data.get("interfase"), data.get("servicio"), data.get("plaza"), "p_cr_plaza");
		pok.runIntefaceWmOneButton10(data.get("interfase"), data.get("servicio"));	
		
//Paso 3 **********************
		addStep("Validar la correcta ejecución de la interface OR1_OG en la tabla WM_LOG_RUN de WMLOG.");
...
//Paso 4 *********************
		addStep("Validar la correcta ejecución de los threads lanzados por la interface OR1_OG en la tabla WM_LOG_THREAD de WMLOG.");
...
//Paso 5 ********************
		addStep("Validar la inserción de los ajustes de inventarios de la plaza "+ data.get("plaza") +" en la tabla FEM_FIF_STG de RETEK.");
		...
		assertFalse(fem, "No se obtiene información de la consulta");
	
//Paso 6 *****************
		addStep("Validar la actualización del estatus (WM_STATUS='E'), en la tabla XXFC_AJUSTE_SALDOS_AUTOMATICOS de ORACLE AR para los ajustes de inventarios para la plaza "+ data.get("plaza") +".");
		System.out.println(GlobalVariables.DB_HOST_RMS_MEX);
		...
		assertFalse(actualizacion, "No se obtiene información de la consulta");
	...
	public String setTestDescription() {
		// TODO Auto-generated method stub
		return "Construido. Registrar en Retek RMS la información de ajustes de inventarios desde Oracle AR.";
	}
	...
	public String setTestFullName() {
		// TODO Auto-generated method stub
		return "ATC_FT_002_OR1_OG_Registrar_Info_Ajustes_Inventarios";
	}
