...
public class MO2_ActualizarRegistrosEnTBL_SOL_IMPUESTOS extends BaseExecution {
	@Test(dataProvider = "data-provider")
	public void ATC_FT_MO2_001_Actualizar_Regs_TBL_SOL_IMPUESTOS(HashMap<String, String> data) throws Exception {
...
//		Paso 4
		String CompDatos = "SELECT PERIODO_ID,DESPACHO_ID, WM_STATUS, WM_RUN_ID, WM_RETRIES, WM_FECHA, WM_ERROR  "
				+ "FROM XXSADEL.TBL_SOL_IMPUESTOS "
				+ "WHERE TRUNC(WM_FECHA) = TRUNC(SYSDATE) "
				+ "AND WM_RUN_ID = '%s' "
				+ "AND PERIODO_ID = '"+data.get("Periodo")+ "' "
				+ "AND DESPACHO_ID = '"+data.get("Despacho")+"' " 
				+ "AND WM_STATUS = 'E'";
...
//**********************************************************Paso 4*************************************************************************************************************			
		...
		addStep("Comprobar que los datos se actualicen correctamente en la tabla TBL_SOL_IMPUESTOS de la "
				+ "BD PORTAL donde WM_FECHA es igual a la fecha actua y WM_STATUS es igual a 'E'.");
		
		System.out.println(GlobalVariables.DB_HOST_FCIASQA);
		String CompDatosFormat = String.format(CompDatos, RunID);
		System.out.println(CompDatosFormat);
		
		SQLResult ExecCompDatos = dbFciasqa.executeQuery(CompDatosFormat);
		...
		assertFalse(ExecCompDatosRes, "La información no ha sido actualizada");
	...
	public String setTestFullName() {
		...
		return "ATC_FT_MO2_001_Actualizar_Regs_TBL_SOL_IMPUESTOS";
	}

	@Override
	public String setTestDescription() {
		// TODO Auto-generated method stub
		return "Construido. Actualiza los registros en la tabla TBL_SOL_IMPUESTOS.S";
	}
