...
public class RE1_OrdenesCompraProveedor_EDIFACT_Peru extends BaseExecution {

	@Test(dataProvider = "data-provider")
	public void ATC_FT_REO1_PERU_002_Ordenes_Compra_EDIFACT(HashMap<String, String> data) throws Exception {
		...
		String validaDoc = "SELECT s.id, s.supplier, s.filetype, i.command_id, i.qualifier, s.status, s.cost, s.surtirporpieza, s.format_type, s.max_attempt \r\n"
				...
				+ "AND s.filetype = 'EDIFACT'  \r\n"
				+ "AND s.supplier = '" + data.get("supplier") + "' \r\n";

		// Paso 2
		String ordenesCedis = " SELECT sup_id, cedis_id, c.cedis, c.retek_cr \r\n"
				+ " FROM POSUSER.wm_edi_supplier_cedis sc INNER JOIN POSUSER.cedis c \r\n" 
				+ " ON (sc.cedis_id = c.id) \r\n"
				+ " WHERE sc.sup_id = '%s' \r\n"
				+ " AND c.cedis = '" + data.get("cedis") + "' \r\n";
...
//************************************************Paso 1********************************************************************************************************************************
		...
		System.out.println(GlobalVariables.DB_HOST_Oiwmqa);// RMS

		System.out.println(validaDoc);

		SQLResult validaDocRes = dbRmsPeru.executeQuery(validaDoc);
...
//*************************************************Paso 2***********************************************************************************************************************
		...
		addStep("Validar que el Proveedor pueda enviar ordenes compra hacia el CEDIS en POSUSER.");

		System.out.println(GlobalVariables.DB_HOST_RMSWMUSERPeru);// Pos

		String ordenesCedis_f = String.format(ordenesCedis, id);

		System.out.println(ordenesCedis_f);

		SQLResult ordenesCedis_r = dbPuserPeru.executeQuery(ordenesCedis_f);

		boolean validaOrdenesCedis = ordenesCedis_r.isEmpty();

		if (!validaOrdenesCedis) {

			testCase.addQueryEvidenceCurrentStep(ordenesCedis_r);

		}

		System.out.println(validaOrdenesCedis);
		assertFalse("No se muestra la configuración del Supplier con la CEDIS", validaOrdenesCedis);
...
//**********************************************************Paso 3*************************************************************************************************************		
		...
		SQLResult ordenesPendientes_r = dbRmsPeru.executeQuery(ordenesPendientes);
...
//********************************************Paso 4**************************************************************************************************************************
		...
		SQLResult detalleOrdenCompra_r = dbRmsPeru.executeQuery(detalleOrdenCompra);
...
//********************************************************Paso 7**********************************************************************************************
		...
		SQLResult threads_r = dbLogPeru.executeQuery(threads_f);
...
//***********************************************Paso 8**********************************************************************************************************
		...
		SQLResult ordenActualizada_r = dbRmsPeru.executeQuery(ordenActualizada_f);
...
//*****************************************Paso 9*****************************************************************************************************************
		...
		SQLResult registroDoc_r = dbRmsPeru.executeQuery(ordenActualizada_f);
	...
	public String setTestFullName() {
		...
		return "ATC_FT_REO1_PERU_002_Ordenes_Compra_EDIFACT";
	}

	@Override
	public String setTestDescription() {
		// TODO Auto-generated method stub
		return "Construido. Envío de órdenes de compra a proveedores EDIFACT.";
	}
