...
public class CT1_CL_ActualizaWM_RETRIES_BUZONrunBuzon extends BaseExecution {

	@Test(dataProvider = "data-provider")
	public void ATC_FT_004_CT1_CL_Verificar_procesamiento_actualizar_WM_RETRIES_BUZON_runBuzon(HashMap<String, String> data) throws Exception {
		...
		String ValidaRetriesyStatus = "SELECT ID,CR_PLAZA, CR_TIENDA, WM_STATUS_BUZON , WM_RETRIES_BUZON "
				+ "  FROM WM_BUZONES_T_TIENDAS " + " WHERE WM_STATUS_BUZON = 'L' " + "    AND CR_PLAZA = '"
				+ data.get("plaza") + "' \r\n" + "    AND CR_TIENDA = '" + data.get("tienda") + "'\r\n"
				+ "  ORDER BY CREATION_DATE";

		// UPDATE
		String UpdateRetriesBuzon = "UPDATE WM_BUZONES_T_TIENDAS " + " SET WM_RETRIES_BUZON  = 1 WHERE ID = '%s'";
...
//			" ORDER BY START_DT DESC) where rownum <=1";
		...
		String tdcIntegrationServerFormat = "select * from (SELECT run_id,interface, start_dt, end_dt, status, server, (END_DT - START_DT)*24*60 "
				+ "FROM WMLOG.WM_LOG_RUN " + "WHERE INTERFACE = 'CT1_BUZON' "
				+ " ORDER BY START_DT DESC) where rownum <=1";

		String ValidaError = "select * from (SELECT run_id,interface, start_dt, end_dt, status, server, (END_DT - START_DT)*24*60 "
				+ "FROM WMLOG.WM_LOG_RUN " + "WHERE INTERFACE = 'CT1_BUZON' " + "AND STATUS = 'E' "
				+ " ORDER BY START_DT DESC) where rownum <=1";
...
//						     "AND TRUNC(WM_FECHA_PROC) = TRUNC(SYSDATE)";
		...
		// Usar esta para pruebas sin ejecutar servicio (RunBuzon)
		String validaRetries = "SELECT ID,CR_PLAZA, CR_TIENDA, WM_STATUS_BUZON , WM_RETRIES_BUZON "
				+ "FROM WM_BUZONES_T_TIENDAS " + "WHERE WM_RETRIES_BUZON = 2 " + "AND CR_PLAZA = '" + data.get("plaza")
				+ "' " + "AND CR_TIENDA = '" + data.get("tienda") + "'";
...
//**********************************************Paso 1	****************************************************************************
//				Verificar que el directorio existe en el filesystem correspondiente a chile
//				PATH=/u01/posuser/FEMSA_OXXO/POS/[PLAZA]/[TIENDA]/ 
//			    DIRECTORIO=backup,working,recovery,outbox,duplicate.

		// utilizar ip y puerto de servidor FTP chile

		addStep("Verificar que el directorio existe en el filesystem, DIRECTORIO=backup,working,recovery,outbox,duplicate.");
		String IpFTP = "";// utilizar ip y puerto de servidor FTP chile
		Integer Port = 21;
		boolean Res = true;
		FTPClient ftpClient = null;
		(ftpClient = new FTPClient()).connect(IpFTP, Port);

		ftpClient.login("posuser", "posuser");//utilizar usuario y contrase�a de servidor FTP chile

		String Ruta = "/u01/posuser/FEMSA_OXXO/POS/" + data.get("plaza") + "/" + data.get("tienda");

		// ********************VALIDA ruta
		// /u01/posuser/FEMSA_OXXO/POS/plaza/tinda******************************************************
...
//				 SQLResult commit = executeQuery(dbCNTCL,comm);
//				 
//				
//				 
...
//**************************************************Paso 7 ***************************************************************************		
	...
	public String setTestDescription() {
		// TODO Auto-generated method stub
		return "Construida. Verificar el procesamiento de la interfaz, actualizar los reintentos WM_RETRIES++";
	}
	...
	public String setTestFullName() {
		// TODO Auto-generated method stub
		return "ATC_FT_004_CT1_CL_Verificar_procesamiento_actualizar_WM_RETRIES_BUZON_runBuzon";
	}
