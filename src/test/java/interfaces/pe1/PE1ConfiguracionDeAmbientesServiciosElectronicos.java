...
public class PE1ConfiguracionDeAmbientesServiciosElectronicos extends BaseExecution {
	...
	public void ATC_FT_004_PE1_PE1ConfiguracionDeAmbientesServiciosElectronicos(HashMap<String, String> data) throws Exception {
		...
		String query_switch="select * from TPEUSER.TAE_ROUTING where carrier='"+data.get("carrier")+"' and plaza='"+data.get("plaza")+"'";
	...
	//paso 1
		testCase.addBoldTextEvidenceCurrentStep("Verificar que el carrier sea valido verificando en la tabla tpeuser.tae_carrier.");
		SQLResult validaCarrierRes = dbFCTAEQA.executeQuery(query_carrier);
		...
		testCase.addBoldTextEvidenceCurrentStep("Validar que la tienda este configurada con el carrier solicitado en la tabla tpeuser.tae_routing.");

		SQLResult validaTiendaRes = dbFCTAEQA.executeQuery(query_routing);
		...
		if (validaTiendaBool == false) {
			...
			testCase.addTextEvidenceCurrentStep("-La plaza: " + data.get("plaza") + " cuenta con el carrier ingresado "+ data.get("carrier")+", por ende esta configurado.");
		...
		testCase.addTextEvidenceCurrentStep("Nota: Se consigue la configuracion de los telefonos mediante la lista de telefonos validos.");
		String resp = null;
		String[][] array = null;
		...
		//paso 5
		testCase.addBoldTextEvidenceCurrentStep("Consultar el swtich configurado con el carrier y plaza ingresados, con la siguiente consulta en la tabla TPEUSER.TAE_ROUTING");
		SQLResult validaSwitch = dbFCTPEQA.executeQuery(query_switch);
		...
		if (validaSwitchBOl == false) {
			...
			testCase.addTextEvidenceCurrentStep("-El carrier y la plaza estan configurados con el siguiente switch=: "+carrierSwitch+".");
			//testCase.addQueryEvidenceCurrentStep(validaSwitch);
		} else {
			System.out.print("Switch no encontrado");
			testCase.addTextEvidenceCurrentStep("-No se encontro un switch configurado para el carrier y plaza introducidos.");
		}
		
		//paso 6
		testCase.addBoldTextEvidenceCurrentStep("Buscar el switch en Integration Server en el apartado de global variables.");
		// utileria
	...
	//*********************************************************************************************FIN RESUMEN********************************************************************************************* 
		...
		testCase.addTextEvidenceCurrentStep("Conexion a bd correcta: " + GlobalVariables.DB_HOST_FCTAEQA + " usuario: "
				+ GlobalVariables.DB_USER_FCTAEQA);
		testCase.addTextEvidenceCurrentStep("Conexion exitosa: " + dbFCTAEQA.getConn());
	...
	//paso 1
		...
		//paso 4
		addStep("Conectarse a la base de datos TPEUSER.");
		...
		testCase.addTextEvidenceCurrentStep("Conexion exitosa: " + dbFCTPEQA.getConn());
	 ...
	 try {
	     ...
	     if(sshConnector.isConected()) {
				...
				if(resumen==true) {
					...
					testCase.addBoldTextEvidenceCurrentStep("Buscar la configuracion del channel del banco '"+switchConf+"'.");
	...
	public String setTestFullName() {
		return "ATC_FT_004_PE1_PE1ConfiguracionDeAmbientesServiciosElectronicos";
	}

	@Override
	public String setTestDescription() {
		return "Construido. Valida la configuracion de ambientes para pruebas con telefonos";
	}
