...
public class OS1_Polizas_F extends BaseExecution {	

	@Test(dataProvider = "data-provider")
	public void ATC_FT_OS1_001_Polizas_F(HashMap<String, String> data) throws Exception {
...
/** UTILERIA *********************************************************************/
		 ...
		 * Validar inserción de polizas en SAP_RPY_DETL para BUKRS F099
		 */

/** VARIABLES *********************************************************************/

		// Paso 1
		String SelectS = "SELECT ID, TARGET_ID, DOCNUM, GSBER, RUNID, SEQNO, CREATE_DATE, ( SELECT DOCLIN FROM SAPUSER.SAP_PYR_DETL T3 WHERE T3.SID_ID = T1.ID AND ROWNUM < 2 ) AS DOCLIN, T2.BUKRS" + 
				" FROM SAPUSER.SAP_INBOUND_DOCS T1, SAPUSER.SAP_PYR_HEADER T2" + 
				" WHERE T1.ID = T2.SID_ID AND T2.TARGET_SYS = 'O'" + 
				" AND T1.STATUS = 'S' AND T2.BUKRS = '" + data.get("bukrs") + "' AND ROWNUM < 10 order by target_id asc ";
...
/** PASOS DEL CASO DE PRUEBA *********************************************************************/	
		...
		addStep("Tener información en la tabla SAP_INBOUND_DOCS de SAPUSER con estatus S.");
		...
		addStep("Tener información de las pólizas en EBS en la tabla GL_INTERFACE con estatus diferente de NEW.");
		...
		addStep("Validar que se actualiza correctamente la información de la tabla SAP_INBOUND_DOCS en SAPUSER con estatus F.");
	...
	public String setTestFullName() {
		...
		return "ATC_FT_OS1_001_Polizas_F";
	}
