...
public class ATC_FT_007_OE2_CL_GeneracionExitosaComprobanteFactura01Electronica extends BaseExecution {

	@Test(dataProvider = "data-provider")
	public void ATC_FT_007_OE2_CL_GeneracionExitosaComprobanteFactura01Electronica_test(HashMap<String, String> data) throws Exception {
		 ...
		 * @author Brandon Ruiz.
		 * @date   30/01/2023.
		 * @cp MTC-FT-014 OE2_CL Generacion de forma electronica exitosa de comprobante Factura(01) por medio de la interface OE2_CL
		 * @projectname Actualizacion Tecnologica Webmethods
		 *
		 */
		
		/** Utilerias *********************************************************************/
		String dbLogHost = GlobalVariables.DB_HOST_LogChile;
		...
		String queryPaso2 = "SELECT ID_FACTURA_DIGITAL, TIPO_COMPROBANTE, WM_ESTATUS FROM  XXFC.XXFC_FACTURA_DIGITAL \r\n"
				+ "WHERE WM_ESTATUS = 'L' AND TIPO_COMPROBANTE = '"+comprobante+"' AND ROWNUM <=10";
		String errorPaso2 = "No existen facturas con estatus: L";
		...
		String queryPaso5 = "SELECT * FROM WM_LOG_RUN\r\n"
				+ "WHERE ROWNUM <=10";
		SQLResult resultPaso5;
		
		String queryPaso6 = "SELECT RUN_ID, INTERFACE, START_DT, STATUS, SERVER FROM WM_LOG_RUN  \r\n"
				+ "WHERE INTERFACE LIKE '%OE2_CL%' AND STATUS = 'S' \r\n"
				+ "AND START_DT >= TRUNC(SYSDATE) AND ROWNUM <=10\r\n"
				+ "ORDER BY START_DT DESC";
		String errorPaso6 = "No se encontro ninguna ejecucion reciente con status: S";
		...
		/** Paso 1******************************************************************************************************************/
		step("Establecer conexion de EBS Chile (OIEBSBDQ) con usuario APPSVIEW");
		o.logHost(dbEbsHost);
		...
		/** Paso 2******************************************************************************************************************/
		step("Consultar la tabla XXFC.XXFC_FACTURA_DIGITAL para validar "
				+ "las facturas global mensual de ventas registradas en POS para la Plaza y la Tienda "
				+ "en la tabla de Oracle a WM_STATUS = 'L'");
		o.logHost(dbEbsHost);
		...
		step("Consultar la tabla XXFC_FACTURA_DIGITAL_LINES para validar el detalle de las facturas global mensual "
				+ "de ventas registradas en POS para la Plaza y la Tienda en la tabla de Oracle");
		...
		errorPaso3 = "No se encontro ninguna factura con id_factura_digital: "+id_factura;
		...
		/** Paso 5******************************************************************************************************************/
		step("Establecer conexion a la BD FCWMLQA de OI_Chile");
		o.logHost(dbLogHost);
		resultPaso5 = ejecutaQuery(queryPaso5, dbLog);
		...
		/** Paso 6******************************************************************************************************************/
		step("Validar la correcta ejecucion de la interface OE2 en la tabla WM_LOG_RUN de WMLOG");
		resultPaso6 = ejecutaQuery(queryPaso6, dbLog);
		...
		/** Paso 7******************************************************************************************************************/
		step("Validar la correcta ejecucion de los Threads lanzados por la interface OE2 en la tabla WM_LOG_THREAD de WMLOG");
		//Segun evidencia manual, la interfaz no genera threads
		queryPaso7 = "SELECT THREAD_ID, NAME, START_DT, STATUS FROM WM_LOG_THREAD \r\n"
				+ "WHERE PARENT_ID = '"+run_id+"'";
		errorPaso7 = "No se encontro ningun thread con parent_id: "+run_id;
		resultPaso7 = ejecutaQuery(queryPaso7, dbLog);
		...
		/** Paso 8******************************************************************************************************************/
		step("Ejecutar la siguiente consulta para comprobar que no se hayan generado errores de la ejecucion de la interfaz");
		queryPaso8 = "SELECT ERROR_ID, ERROR_DATE, DESCRIPTION FROM WMLOG.WM_LOG_ERROR \r\n"
				+ "WHERE RUN_ID='"+run_id+"'";
		errorPaso8 = "Se encontraron errores en la ejecucion de la interfaz";
		resultPaso8 = ejecutaQuery(queryPaso8, dbLog);
		o.log("Consulta ejecutada: "+queryPaso8);
		if (!resultPaso8.isEmpty()) {
			o.mensajeError(errorPaso8);
		}else {
			o.log("No se generaron errores para la ejecucion de la interfaz");
		}

		/** Paso 9******************************************************************************************************************/
		step("Validar la actualizacion del estatus (WM_STATUS = 'S') y los campos referentes a la factura digital "
				+ "en la tabla XXFC.XXFC_FACTURA_DIGITAL de Oracle");
		queryPaso9 = "SELECT ID_FACTURA_DIGITAL, TIPO_COMPROBANTE, WM_ESTATUS FROM  XXFC.XXFC_FACTURA_DIGITAL\r\n"
				+ "WHERE ID_FACTURA_DIGITAL = '"+id_factura+"' AND WM_ESTATUS = 'S'";
		errorPaso9 = "El registro con id_factura_digital: "+id_factura+" tiene un estatus distinto al esperado";
	...
	public String setTestDescription() {
		...
		return "Prueba de regresion para comprobar la no afectacion en la funcionalidad principal de la interface "
				+ "FEMSA_OE2_CL para generar la forma electronica de comprobante Factura(01) con la informacion "
				+ "de Oracle Peru y la respuesta Exitosa por parte del proveedor ACEPTA, al ser migrada la interface "
				+ "de WM9.9 a WM10.5";
	}
	...
	public String setTestFullName() {
		// TODO Auto-generated method stub
		return "ATC_FT_007_OE2_CL_GeneracionExitosaComprobanteFactura01Electronica_test";
	}
