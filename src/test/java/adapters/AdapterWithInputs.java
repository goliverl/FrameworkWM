...
public class AdapterWithInputs extends BaseExecution {
	...
	public void test(HashMap<String, String> data) throws Exception {
		...
		String tdcQueryIntegrationServer = "select * from ( SELECT run_id,start_dt,status"
				+ " FROM WMLOG.wm_log_run"
				 + " WHERE interface = '"+data.get("WMRun")+"'" 	
		...
		boolean isOff = pok.poolSwitchAdapter(data.get("adapter1"),data.get("adapter2"),data.get("adapter3"));
		...
		String input = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" + 
				"<VALIDSKUS><HEADER><DESTINATIONAPP>POS</DESTINATIONAPP>"
				+ "<SENDER_ID>10BCA505DL</SENDER_ID><RECEIVER_ID>10BGA50D4N"
				+ "</RECEIVER_ID><NO_RECORDS>1</NO_RECORDS></HEADER><DETAIL>"
				+ "<SKU>9002490100490</SKU></DETAIL></VALIDSKUS>";
		
		System.out.println(input);		

		pok.runIntefaceWmWithInput(data.get("interface"),data.get("servicio"),input,data.get("value"));
	...
	public String setTestFullName() {
		// TODO Auto-generated method stub
		return "Ejecución de interface con adapter apagado";
	}
